...
Every dependency is permissive and GPL-compatible — `age` itself is BSD-3-Clause
— so the combined work can be GPLv3. Their notices are reproduced in
[THIRD_PARTY_LICENSES.md](THIRD_PARTY_LICENSES.md), which ships inside every
package because BSD, MIT and Apache all require it of binary distributions. That
file is generated by `scripts/gen-third-party-licenses.sh` and CI fails if it
drifts.

One thing to know if you ever reconsider the version: `go-reflector` (a Wails
dependency) is **Apache-2.0**, which is compatible with GPL**v3** but *not*
GPLv2. v3 or later is the only GPL that works here.
